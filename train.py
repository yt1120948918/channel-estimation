...
def train(**kwargs):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(opt.gpu_num):
            if opt.model_type == 0:  # 反卷积
                pre_loss, mse = model[0].calculate_loss(inputs, labels)
                g_vars, _, _ = get_vars()
                with tf.control_dependencies(update_ops):
                    train_ops = optimizer.minimize(pre_loss, var_list=g_vars)
            elif opt.model_type == 1:  # 反卷积+可学习pooling
                _, mse, pred = model[0].calculate_loss(inputs, labels, True)
                ...
                pre_loss, mse, pred = model[0].calculate_loss(inputs, labels, True)
                gen_loss, dis_loss = model[1].calculate_loss(pred, labels)
                g_vars, d_vars, l_p_vars = get_vars()
                with tf.control_dependencies(update_ops):
        ...
        with tf.Session(config=config) as sess:
            ...
            summary_path = opt.summary_path + opt.data_path.split('\\')[-1]
            ...
            # 定义训练集dataset
            train_dataset = CSISet(opt.data_path, opt.batch_size, True, state="train")
            ...
            for num in range(opt.num_epoch):
                ...
                for ii, (batch_x, batch_y) in enumerate(train_dataset.get_data()):
                    if opt.model_type == 0 or opt.model_type == 1:
                ...
                # 定义验证集dataset
                validation_dataset = CSISet(opt.data_path, opt.batch_size, True, state="validation")
                validate_mse = 0
                jj = 1
            ...
            plot_path = os.getcwd() + "\\result\\" + opt.data_path.split("\\")[-1]
            utils.plot_fig(train_mse_for_plot, valid_mse_for_plot, plot_path)
            print("训练过程中最小验证误差是%f" % min(valid_mse_for_plot))

            # 保存模型文件
            model_file = opt.model_path + opt.data_path.split('\\')[-1] + '\\' + opt.data_path.split('\\')[-1]
